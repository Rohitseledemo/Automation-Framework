...
public class NewDemoBasePage {
    ...
    public NewDemoBasePage(){
        ...
        this.percentageOfSurchargeSpendDropdown = By.xpath("//div[@class='dropdown']//button[@id='dropdown-basic']");
        ...
        this.trackingNos = By.xpath("//tbody//tr//td[@class='align-middle text-wrap custom-header']" +
...
//        wait.waitForElementToBeClickable(9,mapStates);
    ...
    public WebDriver getBrowser(){
        return BrowserKeeper.getInstance().getBrowserInstance();
    }
    ...
    public void anyDropdownIterator(By dropdown, By dropdownElements, int careerIndex){
        ...
        javaScript.clickWebElement(dateOptions.get(careerIndex));
    }
...
//        }
    ...
    public boolean sortingValidation(List<Float> columnValues) {
        boolean sorted = columnValues.size() == 1;
        for (int i = 0; i < columnValues.size() - 1; i++) {
            sorted = columnValues.get(i) >= columnValues.get(i + 1) || columnValues.get(i) <= columnValues.get(i + 1);
        }
        return sorted;
    }
    ...
    public boolean testDownloadButtonFunctionality2() {
        ...
        String initialValue;
        wait = new WebDriverWaits(this.getBrowser());
        wait.waitForVisibilityOfElement(30, downloadButton);
        try {
...
//            System.out.println("Initial Value - "+initialValue);
    ...
    public boolean trackingNumberValidation() {
        ...
        for (int i = 0; i < columnWebElements.size(); ) {
            javaScript.clickWebElement(columnWebElements.get(i));
            break;
        }
        ...
        for (WebElement element : columnLinks) {
            ...
            try {
                ...
                if (displayed) {
                    ...
                    closeModal.click();
                    wait.waitForInvisibilityOfWebElement(30, closeModal);
                }
            } catch (Exception e) {
